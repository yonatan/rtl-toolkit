...
class CSSJANUSUnitTest(unittest.TestCase):
  ...
  def testBGPositionLength(self):
    testcase = ['.test { background: 1px 20px } .test2 { background: 40px 30px }']
    shouldbe = ['.test { background: 1px 20px ;background-position:right 1px top 20px} .test2 { background: 40px 30px ;background-position:right 40px top 30px}']
    self.assertEqual(shouldbe, cssjanus.ChangeLeftToRightToLeft(testcase))
